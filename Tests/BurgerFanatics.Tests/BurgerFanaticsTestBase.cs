...
using BurgerFanatics.Migrations.SeedData;
...
namespace BurgerFanatics.Tests
{
    public abstract class BurgerFanaticsTestBase : AbpIntegratedTestBase<BurgerFanaticsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected BurgerFanaticsTestBase()
        {
        ...
        protected void UsingDbContext(Action<BurgerFanaticsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BurgerFanaticsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BurgerFanaticsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BurgerFanaticsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BurgerFanaticsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BurgerFanaticsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BurgerFanaticsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BurgerFanaticsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BurgerFanaticsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BurgerFanaticsDbContext, Task<T>> func)
        {
            T result;
